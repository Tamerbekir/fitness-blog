...
const DeletePost = ({ postId, refetch }) => {
  ...
  const handleDeletePost = async () => {
    try {
      ...
      console.error('Error deleting post. Please try again later.', error);
      console.error('ApolloError details:', error.networkError ? error.networkError.result.errors : error.graphQLErrors);
  ...
  return (
    <div>
      ...
      {!deleteForm && (
        <button className='deletePostBtn' type="button" onClick={() => setDeleteForm(true)}>Delete Post</button>
      )}
      ...
      {deleteForm && (
        <>
          ...
          <button className='deletePostBtn' type="button" onClick={handleDeletePost}>Confirm Delete</button>
          <button className='deletePostBtn' type="button" onClick={() => setDeleteForm(false)}>Cancel</button>
        </>
      )}
